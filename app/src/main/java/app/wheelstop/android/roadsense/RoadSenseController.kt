...
import app.wheelstop.android.roadsense.detect.Assessment
import app.wheelstop.android.roadsense.detect.DaemonImuStream
import app.wheelstop.android.roadsense.detect.DetectionCandidate
import app.wheelstop.android.roadsense.detect.EventDetector
import app.wheelstop.android.roadsense.detect.GravityFrame
import app.wheelstop.android.roadsense.detect.HazardClassifier
import app.wheelstop.android.roadsense.detect.PitchSignCalibrator
import app.wheelstop.android.roadsense.detect.GpsRingBuffer
import app.wheelstop.android.roadsense.detect.GyroStats
...
import app.wheelstop.android.roadsense.detect.ImuStream
import app.wheelstop.android.roadsense.detect.RawImuRecorder
import app.wheelstop.android.roadsense.detect.RejectionFilter
import app.wheelstop.android.roadsense.detect.RoadSenseHazard
import app.wheelstop.android.roadsense.detect.SeverityClassifier
import app.wheelstop.android.roadsense.detect.VehicleCalibrator
import app.wheelstop.android.roadsense.detect.VehicleDynamics
import app.wheelstop.android.roadsense.detect.VerticalSample
import app.wheelstop.android.roadsense.detect.ImuFrameCodec
import app.wheelstop.android.roadsense.config.RoadSenseConfig
import app.wheelstop.android.roadsense.label.GroundTruthStore
import app.wheelstop.android.roadsense.sidecar.RoadSenseImuSidecarService
import app.wheelstop.android.roadsense.source.LocationSource
import app.wheelstop.android.roadsense.source.VehicleDataSource
import app.wheelstop.android.roadsense.source.VehicleStateGate
import app.wheelstop.android.roadsense.store.RoadSenseStore
import app.wheelstop.android.roadsense.store.SpatialIndex
import app.wheelstop.android.roadsense.sync.CloudflareEdgeSyncProvider
import app.wheelstop.android.roadsense.sync.DeviceId
import app.wheelstop.android.roadsense.sync.RoadSenseSyncProvider
import app.wheelstop.android.roadsense.sync.TileCursor
...
class RoadSenseController @JvmOverloads constructor(
    ...
    private val plog = app.wheelstop.android.logging.DaemonLogger.getInstance("RoadSense")
    ...
    private val coverage = app.wheelstop.android.roadsense.store.RouteCoverage()
    ...
    private val visualSink = app.wheelstop.android.roadsense.warn.UcmVisualSink(
    ...
    private var enabledListener: app.wheelstop.android.config.UnifiedConfigManager.ConfigChangeListener? = null
    ...
    fun attach() {
        synchronized(lifecycleLock) {
            if (!attached) {
                ...
                val l = object : app.wheelstop.android.config.UnifiedConfigManager.ConfigChangeListener {
                ...
                enabledListener = l
                app.wheelstop.android.config.UnifiedConfigManager.addListener(l)
    ...
    fun detach() {
        synchronized(lifecycleLock) {
            enabledListener?.let {
                try { app.wheelstop.android.config.UnifiedConfigManager.removeListener(it) } catch (_: Throwable) {}
            }
    ...
    fun stop() {
        ...
        app.wheelstop.android.roadsense.overlay.RoadSenseOverlayService.stopFromDaemon()
    ...
    fun onVehicleStatePoll() {
        ...
        val accOn = app.wheelstop.android.monitor.AccMonitor.isAccOn()
        val accAuth = app.wheelstop.android.monitor.AccMonitor.isAccStateAuthoritative()
        ...
        val gm = app.wheelstop.android.monitor.GearMonitor.getInstance()
        val gear = try {
        ...
        if (newRegime == VehicleStateGate.Regime.OFF || !cfgSnap.overlayVisible) {
            ...
            app.wheelstop.android.roadsense.overlay.RoadSenseOverlayService.startFromDaemon()
        }
    ...
    private fun restoreCalibration() {
        try {
            val rs = app.wheelstop.android.config.UnifiedConfigManager.forceReload()
    ...
    private fun vehicleKey(): String =
        try { app.wheelstop.android.byd.BydDataCollector.getInstance().getData()?.vin ?: "" }
        catch (_: Throwable) { "" }
     ...
     */
    private fun maybeSync(cfg: RoadSenseConfig.Snapshot, pose: app.wheelstop.android.roadsense.detect.Pose, now: Long) {
     ...
     */
    private fun runSync(cfg: RoadSenseConfig.Snapshot, pose: app.wheelstop.android.roadsense.detect.Pose, now: Long) {
    ...
    private fun consumePendingConfirmResult(now: Long) {
        ...
        val userSeverityEnum = userSeverity?.let {
            app.wheelstop.android.roadsense.detect.Severity.values().firstOrNull { s -> s.level == it }
        }
        val userTypeEnum = userType?.let {
            app.wheelstop.android.roadsense.detect.HazardType.values().getOrNull(it)
        }
    ...
    private fun maybeMapRoughSection(candidate: DetectionCandidate, now: Long) {
        ...
        val severity = if (jolt >= ROUGH_SECTION_MODERATE_JOLT)
            app.wheelstop.android.roadsense.detect.Severity.MODERATE
        else app.wheelstop.android.roadsense.detect.Severity.MINOR
        val hazard = RoadSenseHazard(
            lat = pose.lat,
            lng = pose.lng,
