...
import app.wheelstop.android.daemon.telegram.CommandContext;
import app.wheelstop.android.daemon.telegram.CommandRouter;
...
public class TelegramBotDaemon {
    ...
    private static boolean waitForBotTokenAndReload() {
        ...
        while (running) {
            ...
            try {
                ...
                String tok = app.wheelstop.android.telegram.config.UnifiedTelegramConfig.getBotToken();
                if (tok != null && !tok.isEmpty()) {
                    botToken = tok;
                    ownerChatId = app.wheelstop.android.telegram.config.UnifiedTelegramConfig.getOwnerChatId();
                    videoUploadsEnabled = app.wheelstop.android.telegram.config.UnifiedTelegramConfig.isVideoUploads();
                    criticalAlertsEnabled = app.wheelstop.android.telegram.config.UnifiedTelegramConfig.isCriticalAlerts();
                    connectivityAlertsEnabled = app.wheelstop.android.telegram.config.UnifiedTelegramConfig.isConnectivity();
                    motionTextEnabled = app.wheelstop.android.telegram.config.UnifiedTelegramConfig.isMotionText();
    ...
    private static boolean isTelegramDaemonCmdline(String cmdline) {
        ...
        return cmdline.contains("telegram_bot_daemon")
                || cmdline.contains("app.wheelstop.android.daemon.TelegramBotDaemon");
    }
    ...
    private static boolean loadConfig() {
        try {
            ...
            app.wheelstop.android.telegram.config.UnifiedTelegramConfig.migrateLegacyIfNeeded();
            ...
            try {
                if (app.wheelstop.android.telegram.config.UnifiedTelegramConfig.reEncryptBotTokenIfLegacy()) {
            ...
            botToken = app.wheelstop.android.telegram.config.UnifiedTelegramConfig.getBotToken();
            if (botToken == null || botToken.isEmpty()) {
                ...
                if (app.wheelstop.android.telegram.config.UnifiedTelegramConfig.botTokenPresentButUndecryptable()) {
    ...
    private static void refreshConfigFromUnified() {
        try {
            ...
            org.json.JSONObject section =
                    app.wheelstop.android.telegram.config.UnifiedTelegramConfig.load();
            if (section == null || section.length() == 0) {
                return;
            }

            String newToken = app.wheelstop.android.telegram.config.UnifiedTelegramConfig.getBotToken();
            long newOwner = app.wheelstop.android.telegram.config.UnifiedTelegramConfig.getOwnerChatId();
            boolean newVideo = app.wheelstop.android.telegram.config.UnifiedTelegramConfig.isVideoUploads();
            boolean newCritical = app.wheelstop.android.telegram.config.UnifiedTelegramConfig.isCriticalAlerts();
            boolean newConnectivity = app.wheelstop.android.telegram.config.UnifiedTelegramConfig.isConnectivity();
            boolean newMotionText = app.wheelstop.android.telegram.config.UnifiedTelegramConfig.isMotionText();
        ...
        try {
            ...
            if (Boolean.TRUE.equals(lastSendUndelivered.get())
                ...
                try {
                    ...
                    boolean spooled =
                            app.wheelstop.android.telegram.TelegramSpool.offer(spoolCmd);
                    if (spooled) {
     ...
     * age-capped by {@link app.wheelstop.android.telegram.TelegramSpool} so a long
    ...
    private static void drainSpooledNotifications() {
        ...
        try {
            int n = app.wheelstop.android.telegram.TelegramSpool.drain(cmd -> {
...
<<<<<<< HEAD:app/src/main/java/app/wheelstop/android/daemon/TelegramBotDaemon.java
...
=======
        ...
        //
>>>>>>> upstream/main:app/src/main/java/com/overdrive/app/daemon/TelegramBotDaemon.java
    ...
    private static void handlePairCommand(long chatId, String username, String firstName, String pin) {
        ...
        try {
            app.wheelstop.android.config.UnifiedConfigManager.forceReload();
            expectedPin = app.wheelstop.android.telegram.config.UnifiedTelegramConfig.getPairPin();
            pinExpiry = app.wheelstop.android.telegram.config.UnifiedTelegramConfig.getPairPinExpiry();
        } catch (Exception e) {
    ...
    private static void clearPairPinFromConfig() {
        try {
            app.wheelstop.android.telegram.config.UnifiedTelegramConfig.clearPairPin();
        } catch (Exception e) {
    ...
    private static boolean saveOwnerToConfig(long chatId, String username, String firstName) {
        try {
            boolean ok = app.wheelstop.android.telegram.config.UnifiedTelegramConfig.setOwner(
