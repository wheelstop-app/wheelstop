...
class BootReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        when (action) {
            Intent.ACTION_POWER_CONNECTED -> {
                try {
                    app.wheelstop.android.monitor.ChargingDetector.getInstance().onPowerConnected()
                } catch (e: Exception) {
            ...
            Intent.ACTION_POWER_DISCONNECTED -> {
                try {
                    app.wheelstop.android.monitor.ChargingDetector.getInstance().onPowerDisconnected()
                } catch (e: Exception) {
        ...
        if (isRecoveryTrigger(action)) {
            try {
                if (java.io.File(app.wheelstop.android.ui.model.ParkedShutdown.MARKER_PATH).exists()) {
        ...
        when (action) {
            ...
            "com.htc.intent.action.QUICKBOOT_POWERON" -> {
                startDaemons(context, action)
                try {
                    val launchIntent = Intent(context, app.wheelstop.android.ui.MainActivity::class.java)
            ...
            Intent.ACTION_MY_PACKAGE_REPLACED -> {
                ...
                try {
                    ...
                    launchIntent.putExtra(
                        app.wheelstop.android.updater.UpdateLifecycle.EXTRA_POST_UPDATE,
                        true,
                    )
    ...
    private fun armBlindSpotIfEnabled(context: Context) {
        try {
            app.wheelstop.android.roadsense.overlay.BlindSpotControl.sync(context)
        } catch (t: Throwable) {
    ...
    private fun startDaemons(context: Context, trigger: String) {
        ...
        try {
            val markerPresent = java.io.File(
                app.wheelstop.android.ui.model.ParkedShutdown.MARKER_PATH).exists()
            if (markerPresent) {
