...
public class VehicleActuatorService extends Service {
    ...
    private static final String CHANNEL_ID = "wheelstop_vehicle_actuator";
    ...
    private void reconcilePersistedEnergyState(long ticket, int attempt) {
        ...
        app.wheelstop.android.byd.VehicleActuatorBridge.PublishedEnergyRead read =
                readPublishedEnergyStateWithRetry(getApplicationContext());
        if (read.status
                != app.wheelstop.android.byd.VehicleActuatorBridge.EnergyReadStatus.VALID
                || read.request == null) {
            if (read.status
                    != app.wheelstop.android.byd.VehicleActuatorBridge.EnergyReadStatus.MISSING) {
                schedulePersistedEnergyReconciliation(ticket, attempt + 1);
            }
            return;
        }
        app.wheelstop.android.byd.VehicleActuatorBridge.PublishedEnergyRequest marker =
                read.request;
        ...
        if (marker.cancelled) {
            if (marker.rollbackPending
                    ...
                    == app.wheelstop.android.byd.VehicleActuatorBridge.ENERGY_ACTUATOR_APP
    ...
    private boolean setEnergyMode(int mode, long sourceGeneration) {
        ...
        synchronized (energyModeQueue) {
            ...
            app.wheelstop.android.byd.VehicleActuatorBridge.PublishedEnergyRead publishedRead =
                    readPublishedEnergyStateWithRetry(getApplicationContext());
            app.wheelstop.android.byd.VehicleActuatorBridge.PublishedEnergyRequest published =
    ...
    private boolean cancelEnergyMode(long sourceGeneration) {
        ...
        boolean persisted =
                app.wheelstop.android.byd.VehicleActuatorBridge.persistEnergyCancellation(
    ...
    private static EnergyWriteResult writeEnergyModeDirect(EnergyHalTask task) {
        ...
        try {
            ...
            if (!task.compensation) {
                ...
                if (claim <= 0) {
                    claim = app.wheelstop.android.byd.VehicleActuatorBridge
     ...
     */
    private static app.wheelstop.android.byd.VehicleActuatorBridge.PublishedEnergyRead
    readPublishedEnergyStateWithRetry(Context context) {
        app.wheelstop.android.byd.VehicleActuatorBridge.PublishedEnergyRead published =
                app.wheelstop.android.byd.VehicleActuatorBridge.readPublishedEnergyState(context);
        if (published.status
                == app.wheelstop.android.byd.VehicleActuatorBridge.EnergyReadStatus.UNREADABLE
