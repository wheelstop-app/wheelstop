...
import app.wheelstop.android.receiver.ScreenOffReceiver
...
class DaemonKeepaliveService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        try {
            if (app.wheelstop.android.config.UnifiedConfigManager.isVehicleOnOnlyMode() &&
                java.io.File(app.wheelstop.android.ui.model.ParkedShutdown.MARKER_PATH).exists() &&
        ...
        val postUpdate = app.wheelstop.android.updater.UpdateLifecycle
        ...
        try {
            app.wheelstop.android.overlay.StatusOverlayService.startIfPermitted(applicationContext)
        } catch (e: Exception) {
    ...
    private fun parkStanddown(appCtx: Context) {
        ...
        try { app.wheelstop.android.overlay.StatusOverlayService.stop(appCtx) } catch (t: Throwable) {
            Log.w(TAG, "parkStanddown: StatusOverlayService.stop failed: ${t.message}")
        }
        try {
            appCtx.stopService(Intent(appCtx,
                app.wheelstop.android.services.LocationSidecarService::class.java))
        } catch (t: Throwable) {
    ...
    private fun reconcileWakeLockToScreenState() {
        try {
            if (!app.wheelstop.android.config.UnifiedConfigManager.isVehicleOnOnlyMode()) return
