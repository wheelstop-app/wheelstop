...
public class PerformanceApiHandler {
    ...
    private static boolean handleDataUsageGet(String path, OutputStream out) throws Exception {
        try {
            ...
            JSONObject usage = app.wheelstop.android.monitor.DataUsageMonitor.getInstance().getUsage(days);
    ...
    private static boolean handleDataUsageToggle(String body, OutputStream out) throws Exception {
        try {
            ...
            boolean ok = app.wheelstop.android.config.UnifiedConfigManager.INSTANCE
    ...
    private static boolean handleDataUsageReset(OutputStream out) throws Exception {
        try {
            boolean ok = app.wheelstop.android.monitor.DataUsageMonitor.getInstance().resetHistory();
    ...
    private static boolean handleSohReset(OutputStream out) throws Exception {
        try {
            ...
            if (sohEst != null) {
                ...
                android.content.Context appCtx = app.wheelstop.android.daemon.CameraDaemon.getAppContext();
            ...
            for (int i = 0; i < cats.length(); i++) {
                ...
                try {
                    switch (cat) {
                        ...
                        case "mediaProximity": {
                            long n = app.wheelstop.android.storage.StorageManager.getInstance()
                        ...
                        case "dataUsage": {
                            boolean ok = app.wheelstop.android.monitor.DataUsageMonitor
