...
public class StreamingApiHandler {
    ...
    private static void handleBsTarget(OutputStream out, String raw) throws Exception {
        ...
        app.wheelstop.android.config.UnifiedConfigManager.setBlindSpotTarget(t);
        GpuSurveillancePipeline pipeline = CameraDaemon.getGpuPipeline();
    ...
    private static void handleBsGeometry(OutputStream out, String path) throws Exception {
        ...
        if (tail.startsWith("size/")) {
            ...
            try {
                ...
                if (pipeline != null && cluster == app.wheelstop.android.config.UnifiedConfigManager.isBlindSpotCluster()) {
        ...
        try {
            ...
            if (collector.isInitialized()) {
                ...
                if (packed >= 0) {
                    ...
                    app.wheelstop.android.byd.BydVehicleData d = collector.getData();
                    if (d != null) {
                        int un = app.wheelstop.android.byd.BydVehicleData.UNAVAILABLE;
    ...
    private static void handleBlindSpotParams(OutputStream out, String tail) throws Exception {
        ...
        float contrast = app.wheelstop.android.config.UnifiedConfigManager.getBlindSpotContrast();
        float sharpen  = app.wheelstop.android.config.UnifiedConfigManager.getBlindSpotSharpen();
        try {
            String[] p = tail.split("/");
