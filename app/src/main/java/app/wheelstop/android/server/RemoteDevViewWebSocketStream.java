...
final class RemoteDevViewWebSocketStream {
    ...
    private static void produceFrames(
        ...
        while (running.get() && !client.isClosed()
            ...
            boolean accessLocked = "LOCKED".equals(
                metadata.optString("pixelCopyResult", ""));
            ...
            if (isNewFrame || accessLocked || (!successful && !hasSuccessfulFrame)) {
                ...
                // A physical-PIN relock is not transient: deliver it so the
                // browser immediately erases its last canvas frame.
                latest.poll();
