...
public class VehicleControlApiHandler {
        ...
        if (wheelHeat == 1 || wheelHeat == 2) {
            ...
            try {
                VehicleCloudSnapshot cs =
                        BydCloudDataProvider.getInstance().getSnapshot();
            ...
            switch (action) {
                ...
                case "step_temp": {
                    ...
                    int areaDefault = (zone == 2)
                            ? BydDataCollector.AC_TEMP_AREA_PASSENGER
                            : BydDataCollector.AC_TEMP_AREA_DRIVER;
                    Integer areaValue = req.has("area") ? jsonInteger(req, "area")
                            : Integer.valueOf(areaDefault);
                    if (areaValue == null
                            || (areaValue.intValue() != BydDataCollector.AC_TEMP_AREA_DRIVER
                            && areaValue.intValue() != BydDataCollector.AC_TEMP_AREA_PASSENGER)) {
            ...
            if (r.outcome == VehicleCommandRouter.Outcome.SUCCESS) {
                if ("power_on".equals(action)) {
                    ...
                    if (powerOnAutoOffMinutes != null && powerOnAutoOffMinutes.intValue() > 0) {
                        AcAutoOffTimer.arm(powerOnAutoOffMinutes.intValue());
                    }
                }
        ...
        try {
            ...
            String kind =
                    RemoteCommunicationPolicy
                            .effectiveKind(
                                    requestedKind,
                                    app.wheelstop.android.communication
    ...
    private static void handlePlayAudio(OutputStream out, String body) throws Exception {
        ...
        try {
            ...
            boolean ok = onScreen
                    ? AudioPlaybackController.playVideoOnScreen(resolved, loop)
    ...
    private static void handleGetChargingSchedule(OutputStream out) throws Exception {
        ...
        try {
            ...
            try {
                ...
                SmartChargeCache.updateFromCloud(cfg.vin, homePage);
                cloudRead = true;
            } catch (BydCloudClient.SmartChargeNotSupportedException e) {
                resp.put("success", true);
                resp.put("supported", false);
            ...
            JSONObject cached = SmartChargeCache.getSnapshot(cfg.vin);
    ...
    private static boolean isValidClimateTemperature(double temp) {
        return !Double.isNaN(temp) && !Double.isInfinite(temp)
                && temp >= BydDataCollector.AC_SETPOINT_MIN_C
                && temp <= BydDataCollector.AC_SETPOINT_MAX_C;
    }
    ...
    private static Integer optionalAutoOffMinutes(JSONObject request) {
        ...
        return value != null && value.intValue() >= 0
                && value.intValue() <= AcAutoOffTimer.MAX_MINUTES
                ? value : null;
    }
