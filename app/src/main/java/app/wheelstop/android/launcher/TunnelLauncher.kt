                ...
                if (isRunning) {
                    ...
                    getTunnelUrl { existingUrl ->
                        if (existingUrl != null) {
                            ...
                            val isPaid = app.wheelstop.android.config.CloudflaredPaidConfig.isPaidVersion()
...
// --retries 20: Keeps trying to reconnect for a long time
            append("$CLOUDFLARED_TMP_PATH ${app.wheelstop.android.config.CloudflaredPaidConfig.getArgs()}")
    ...
    private fun doWaitForTunnelUrlPoll(callback: TunnelCallback, attempt: Int) {
        adbShellExecutor.execute(
            command = app.wheelstop.android.config.CloudflaredPaidConfig.getUrlExtractionCommand(CLOUDFLARED_LOG),
            callback = object : AdbShellExecutor.ShellCallback {
                override fun onSuccess(logContent: String) {
                    val tunnelUrl = app.wheelstop.android.config.CloudflaredPaidConfig.parseUrl(logContent)
    ...
    fun getTunnelUrl(callback: (String?) -> Unit) {
        ...
        adbShellExecutor.execute(
            command = app.wheelstop.android.config.CloudflaredPaidConfig.getGrepUrlCommand(CLOUDFLARED_LOG),
            callback = object : AdbShellExecutor.ShellCallback {
                override fun onSuccess(output: String) {
                    var url = output.trim()
                    if (url.isNotEmpty()) {
