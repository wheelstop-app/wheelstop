...
import app.wheelstop.android.telemetry.OverlayBitmapRenderer;
import app.wheelstop.android.telemetry.TelemetryDataCollector;
import app.wheelstop.android.telemetry.TelemetrySnapshot;
...
public class OemDashcamPipeline {
    ...
    private volatile app.wheelstop.android.streaming.GpuStreamScaler streamScalerForOemPublish;
    ...
    private volatile app.wheelstop.android.telemetry.TelemetryFields overlayFields =
        app.wheelstop.android.telemetry.TelemetryFields.legacyDefault();
     ...
     */
    public void setStreamScalerForOemPublish(app.wheelstop.android.streaming.GpuStreamScaler scaler) {
    ...
    private void reapplyIdleStrideAfterStop() {
        try {
            ...
            boolean surveillanceAxis = !app.wheelstop.android.monitor.AccMonitor.isAccOn();
            int target;
    ...
    private void handleWriterAbort(String reason) {
        if (recording.compareAndSet(true, false)) {
            ...
            // {@link app.wheelstop.android.storage.StorageManager#sweepOrphanTempFiles}
    ...
    private void reconcileTelemetryHold() {
        ...
        if (telemetryCollector == null) {
            ...
            app.wheelstop.android.byd.BydDataCollector.setOverlayBeamDemand("oem", false);
            return;
        }
        ...
        if (shouldHold) {
            telemetryCollector.setOverlayFieldDemand(
                "oem",
                overlayFields.has(app.wheelstop.android.telemetry.TelemetryFields.Field.TURN_SIGNALS),
                overlayFields.hasAny(
                    app.wheelstop.android.telemetry.TelemetryFields.Field.SEATBELT_DRIVER,
                    app.wheelstop.android.telemetry.TelemetryFields.Field.SEATBELT_PASSENGER));
        }
        ...
        app.wheelstop.android.byd.BydDataCollector.setOverlayBeamDemand(
            "oem",
            shouldHold && overlayFields.hasAny(
                app.wheelstop.android.telemetry.TelemetryFields.Field.LOW_BEAM,
                app.wheelstop.android.telemetry.TelemetryFields.Field.HIGH_BEAM));
     ...
     */
    public void setOverlayFields(app.wheelstop.android.telemetry.TelemetryFields fields) {
        ...
        try {
            ...
            if (fps != oldFps || bitrate != oldBitrate) {
                logger.info("OEM axis profile re-applied live: fps " + oldFps + "→" + fps
                    ...
                    + " Mbps (accOn=" + app.wheelstop.android.monitor.AccMonitor.isAccOn() + ")");
            }
        } catch (Throwable t) {
    ...
    private int applyBitrateBudgetCap(int requested, JSONObject oem,
        ...
        try {
            ...
            int panoBps;
            try {
                app.wheelstop.android.surveillance.GpuPipelineConfig.RecordingQuality q =
                    app.wheelstop.android.surveillance.GpuPipelineConfig.RecordingQuality
                        .fromString(panoQuality);
                app.wheelstop.android.surveillance.GpuPipelineConfig.VideoCodec c =
    ...
    private void initEglAndEncoder() throws Exception {
        ...
        runOnGlThreadAndWait(() -> {
            try {
                ...
                passthroughProgram = app.wheelstop.android.camera.GlUtil
                ...
                overlayProgramId = app.wheelstop.android.camera.GlUtil
                ...
                if (overlayProgramId == 0) {
                    ...
                    overlayVertexBuffer = app.wheelstop.android.camera.GlUtil.createFloatBuffer(overlayVerts);
                    overlayTexCoordBuffer = app.wheelstop.android.camera.GlUtil.createFloatBuffer(overlayUv);
    ...
    private void renderLoop() {
        ...
        while (running.get()) {
            try {
                ...
                app.wheelstop.android.streaming.GpuStreamScaler scaler =
