...
object RecordingScanner {
    ...
    private const val LEGACY_BASE_DIR = "/storage/emulated/0/Android/data/app.wheelstop.android/files"
    ...
    fun getRecordingsDir(context: Context): File {
        return app.wheelstop.android.storage.StorageManager.getInstance().recordingsDir
    }
    ...
    fun getSentryEventsDir(context: Context): File {
        return app.wheelstop.android.storage.StorageManager.getInstance().surveillanceDir
    }
    ...
    fun getProximityEventsDir(context: Context): File {
        return app.wheelstop.android.storage.StorageManager.getInstance().proximityDir
    }
    ...
    private fun scanRecordingsDirect(context: Context): List<RecordingFile> {
        ...
        val sm = app.wheelstop.android.storage.StorageManager.getInstance()
    ...
    private fun scanDirectoryDedup(dir: File, type: RecordingFile.RecordingType,
        ...
        val files = try {
            app.wheelstop.android.storage.StorageManager.getInstance().listMp4Files(dir)
        } catch (e: Exception) {
