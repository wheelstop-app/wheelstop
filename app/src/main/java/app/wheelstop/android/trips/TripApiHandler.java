...
public class TripApiHandler {
    ...
    private JSONObject handleStartRecoverTrips() {
        ...
        final app.wheelstop.android.storage.StorageManager sm;
        try {
            sm = app.wheelstop.android.storage.StorageManager.getInstance();
        } catch (Throwable e) {
        ...
        try {
            app.wheelstop.android.storage.StorageManager.StorageType configured = sm.getTripsStorageType();
            if (configured == app.wheelstop.android.storage.StorageManager.StorageType.SD_CARD && !sm.isSdCardAvailable()) {
                configuredExternalButUnmounted = true;
            } else if (configured == app.wheelstop.android.storage.StorageManager.StorageType.USB && !sm.isUsbAvailable()) {
                configuredExternalButUnmounted = true;
            }
    ...
    private JSONObject handleGetRange() {
        ...
        try {
            ...
            double currentSoc = 0;
            try {
                app.wheelstop.android.monitor.BatterySocData socData =
            ...
            if (estimate != null) {
                ...
                try {
                    app.wheelstop.android.monitor.DrivingRangeData rangeData =
    ...
    private JSONObject handleGetConfig() {
        ...
        try {
            ...
            try {
                org.json.JSONObject lastCharge = app.wheelstop.android.monitor.SocHistoryDatabase
