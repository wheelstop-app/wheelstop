...
public class HardwareEventRecorderGpu {
    ...
    private volatile long segmentDurationMs = app.wheelstop.android.util.Constants.SEGMENT_DURATION_MS;
        ...
        if (tempFile != null && tempFile.exists()) {
            if (!recordingBroken && recordedFrames > 0 && tempFile.length() > 1024) {
                ...
                if (tempFile.renameTo(finalFile)) {
                    ...
                    try {
                        app.wheelstop.android.storage.StorageManager.getInstance().onFileSaved(finalFile);
                    } catch (Exception e) {
                    ...
                    try {
                        app.wheelstop.android.server.RecordingsIndex.getInstance().upsert(finalFile);
                    } catch (Throwable e) {
    ...
    private static long deleteSegmentSidecars(File mp4File) {
        ...
        try {
            app.wheelstop.android.server.RecordingsApiHandler.invalidateRecordingCache(
