...
public class GpuSurveillancePipeline {
    ...
    private boolean disableStreamingIfIdle(
            WebSocketStreamServer idleServer) {
        streamLifecycleLock.lock();
        try {
     ...
     * ✕ ({@link app.wheelstop.android.overlay.StatusOverlayService}) attaches to.
     *
    ...
    public boolean dismissBlindSpotCard() {
        ...
        try {
            ...
            preview = UnifiedConfigManager.forceReload()
                    .optJSONObject("blindspot") != null
                && UnifiedConfigManager.getBlindSpot()
        ...
        if (preview) {
            ...
            try {
                UnifiedConfigManager.setBlindSpotValues(
    ...
    private void enableBlindSpotInternal() throws Exception {
        ...
        if (bsLayer != null) {
            ...
            boolean showNow = bsLayerVisible && bsGateBlockReason(
                UnifiedConfigManager.getBlindSpot(), false).isEmpty();
            if (showNow) {
        ...
        if (camViewActive) {
            ...
            Thread cvOff = new Thread(() -> {
                try {
                    ...
                    UnifiedConfigManager.setCamViewValues(off);
                } catch (Throwable ignored) {}
            }, "CamViewFlagClear");
     ...
     *       unreadable speed: {@link app.wheelstop.android.byd.BydDataCollector#readCurrentSpeedKmh()}
     ...
     *       gearbox reports R. Only trusted when {@link app.wheelstop.android.monitor.GearMonitor}
    ...
    private String bsGateBlockReason(org.json.JSONObject bs, boolean holding) {
        try {
            ...
            int min = UnifiedConfigManager
                .clampBsSpeedBound(bs.optInt("minSpeedKmh", 0));
            int max = UnifiedConfigManager
            ...
            if (min > 0 || max > 0) {
                double kmh = BydDataCollector.getInstance().readCurrentSpeedKmh();
            ...
            if (bs.optBoolean("suppressInReverse", false)) {
                ...
                if (gm.isActive() && gm.getCurrentGear() == GearMonitor.GEAR_R) {
                    return "reverse";
                }
            }
        } catch (Throwable t) {
    ...
    private boolean bsGateVerdictLive() {
        ...
        try {
            return !UnifiedConfigManager.getBlindSpot()
                .optBoolean("debugPreview", false);
        } catch (Throwable t) {
            return false;
        }
        ...
        try {
            ...
            if (!camViewCluster && blindSpotEnabled) {
                String block = bsGateBlockReason(
                    UnifiedConfigManager.getBlindSpot(), true);
                if (!block.isEmpty()) {
    ...
    public void enableCamView(int mode, String target, int autoHideSec) throws Exception {
        ...
        try {
            ...
            camViewMode = StreamingApiHandler.isCamViewMode(mode) ? mode : 0;
    ...
    /**
     * Fire an {@code am broadcast} telling the app-process close-button overlay
     * (the blind-spot ✕ hosted in {@link app.wheelstop.android.overlay.StatusOverlayService})
    ...
    public static final class ExternalStreamClientSubscription {
        ...
        private final WebSocketStreamServer server;
    ...
    public ExternalStreamClientSubscription registerExternalStreamClient(
        ...
        try {
            ...
            WebSocketStreamServer server = wsStreamServer;
