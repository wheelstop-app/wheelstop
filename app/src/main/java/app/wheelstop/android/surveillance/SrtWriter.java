 ...
 *
 * <p>i18n: text is resolved through {@code app.wheelstop.android.server.Messages}
...
public final class SrtWriter {
     ...
     * substitution. The catalog ({@code app.wheelstop.android.server.Messages})
    ...
    private static String lookupCatalog(String key, String locale) {
        try {
            Class<?> cls = Class.forName("app.wheelstop.android.server.Messages");
    ...
    private static String resolveCurrentLocale() {
        try {
            Class<?> cls = Class.forName("app.wheelstop.android.server.LocaleManager");
