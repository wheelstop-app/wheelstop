...
public class SafeLocationManager {
    ...
    private void invalidateGeoCacheNear(double lat, double lng, int radiusM) {
        try {
            Class<?> cls = Class.forName("app.wheelstop.android.geo.GeoCache");
    ...
    private void publishLocationEvent(String zoneName) {
        ...
        try {
            app.wheelstop.android.automation.Automations.update(
                    app.wheelstop.android.automation.condition.BydEvent.LOCATION_ZONE, value);
        } catch (Throwable ignored) {}
    }
    ...
    private void onLeftSafeZone() {
        ...
        if (CameraDaemon.isSafeZoneSuppressed()) {
            ...
            if (app.wheelstop.android.config.UnifiedConfigManager.isSurveillanceEnabled()) {
