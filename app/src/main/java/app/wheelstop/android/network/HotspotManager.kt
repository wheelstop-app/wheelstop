...
object HotspotManager {
    ...
    fun snapshot(): org.json.JSONObject {
        ...
        out.put("proxyPort", app.wheelstop.android.daemon.proxy.ProxyConfiguration.PROXY_PORT)
    ...
    private fun syncClientTunnel() {
        ...
        val cfg = try {
            app.wheelstop.android.daemon.proxy.ProxyConfiguration.createConfig(
        ...
        val cfgPath = app.wheelstop.android.daemon.proxy.ProxyConfiguration.CONFIG_PATH
        val bin = app.wheelstop.android.daemon.proxy.ProxyConfiguration.SINGBOX_PATH
    ...
    private fun notifyCapReached(capMb: Long, usedBytes: Long) {
        try {
            ...
            app.wheelstop.android.notifications.NotificationBus.get().publish(
                app.wheelstop.android.notifications.NotificationEvent(
    ...
    private fun applySystemWideProxy(on: Boolean) {
        val port = app.wheelstop.android.daemon.proxy.ProxyConfiguration.PROXY_PORT
        val exclusions = try {
            app.wheelstop.android.daemon.proxy.ProxyConfiguration.PROXY_EXCLUSIONS
        } catch (t: Throwable) { "" }
    ...
    private fun runShell(command: String): String? {
        ...
        try {
            val launcher = app.wheelstop.android.launcher.AdbDaemonLauncher(ctx)
            launcher.executeShellCommand(command, object : app.wheelstop.android.launcher.AdbDaemonLauncher.LaunchCallback {
    ...
    private fun runShellScript(body: String): String? {
        ...
        try {
            val launcher = app.wheelstop.android.launcher.AdbDaemonLauncher(ctx)
            launcher.executeShellScript(body, object : app.wheelstop.android.launcher.AdbDaemonLauncher.LaunchCallback {
                override fun onLog(message: String) { sb.append(message) }
                override fun onLaunched() { latch.countDown() }
                override fun onError(error: String) {
