...
> - Real actuation is therefore **not** local-HAL: the **HTTP** control path (`/api/vehicle/*`) routes through `VehicleCommandRouter`, which can use the **BYD cloud REST API** (`/control/remoteControl`) authenticated with your stored BYD-cloud login, plus whatever non-signature property surface exists. `GET` permissions and shell-grantable ones (e.g. `WRITE_SECURE_SETTINGS`, which carries the development flag) *are* granted; the sig-protected `*_SET` HAL writes are not.
> - **Path asymmetry worth noting (per Codex PR review):** the **MQTT** command path is *not* the same — `MqttCommandRouter` calls `VehicleCommandRouter.executeSdkOnly`, i.e. **SDK/HAL-only, no cloud fallback**. So MQTT-injected commands (F7) hit exactly the signature-denied HAL writes and, on a stock unsigned unit, do **not** actuate — a tighter bound than the HTTP path. See the actuation-scope correction added to [doc 06 F7](findings/06-mqtt.md#f7).
>
