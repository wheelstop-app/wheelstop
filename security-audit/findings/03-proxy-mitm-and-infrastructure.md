...
> **Mitigating nuance (verified):** the device-**global** HTTP proxy path — which would have routed *all* head-unit traffic, not just OverDrive's — is **disabled** in this build; `setupSystemProxy()` now only clears stale proxies rather than setting one. So F2 is *app-scoped* MITM, not literally every packet on the unit. It still covers everything OverDrive sends **through its `ProxyHelper` clients** — including your BYD-cloud login and the update download — but **not** the Telegram bot daemon, which (per the scope correction above) reads the device-global proxy and therefore connects directly unless the owner separately sets one.
